...
| 4 | [parallel-programming-playground](https://github.com/Skippia/parallel-programming-playground) | TypeScript | • Mutex (3 реализации + deadlock/livelock), Semaphore (бинарный/счётный + atomics), бенчмарки Atomics<br>• WorkerPool, async-примитивы (async-mutex, async-semaphore, barrier, bounded-channel, rate-limiter, rw-lock)<br>• Распределённая блокировка Redis, lock-free CAS-счётчик | CPU-интенсивные задачи (изображения/видео), rate limiting, пулы соединений, распределённые блокировки |
...
| # | Репозиторий | Язык | Описание | Варианты использования |
|---|-------------|------|----------|------------------------|
| 19 | [miscellaneous-playground](https://github.com/Skippia/miscellaneous-playground) | JavaScript | • 10 отдельных примеров: shadowing undefined, short-circuit side effects, autoboxing/prototype pollution<br>• Оператор запятая, потеря async stack trace, неэнумерабельная сериализация Error<br>• Async race conditions | Код-ревью, аудит безопасности (prototype pollution), отладка async |
